...
var removeDuplicates = function(nums) {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        // Step 3:
        // Compare the current element nums[i]
        // with the last unique element nums[x].
        //
        // Since the array is sorted:
        // - nums[x] == nums[i] → duplicate → do nothing
        // - nums[x] < nums[i]  → new unique element found
        if (nums[x] < nums[i]) {

            // Step 4:
            // Move x to the next position because
            // we have found a new unique element.
            x = x + 1;

            // Step 5:
            // Put the newly found unique element
            // at position x.
            nums[x] = nums[i];
        }
    }
    ...
    // Step 6:
    // x represents the index of the last unique element.
